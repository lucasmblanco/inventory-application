...
async function genreCreate(name, position) {
  const genre = new Genre({ name: name });
  await genre.save();
  genres[position] = genre
  console.log(`Added genre: ${name}`);
}

async function artistCreate(name, formation_year, disbandment_year, biography, photo, position) {
...
async function albumCreate(title, artist, cover, release_year, genre, position) {
...
async function createStatuses() {
  ...
  await Promise.all([
    statusCreate(artists[0], 83678669, 20207945, 0),
    statusCreate(artists[1], 35157417, 23039089, 1),
    statusCreate(artists[2], 68044487, 73571749, 2),
    statusCreate(artists[3], 25335881, 24245602, 3),
    statusCreate(artists[4], 83870715, 70928899, 4),
  ]);
}